...
const LIST_HEAD = [
  { align: 'left', label: 'Kode' },
  { align: 'right', label: 'Nama' },
  { align: 'right', label: 'Kategory' },
  { align: 'right', label: 'Harga' },
  { align: 'left', label: '' },
];
...
export default function TableList({ listData, fatching }) {
  ...
  const handleDelete = async (id) => {
    await axiosInstance
      .delete('/api/pelanggan', { params: { id } })
      .then((res) => {
        console.log(res);
            ...
            {listData
              ...
              .map((data, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell align='right'>{data.domisili}</TableCell>
                  <TableCell align='right'>{data.jenis_kelamin}</TableCell>
